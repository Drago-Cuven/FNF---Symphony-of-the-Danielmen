...
static var previousScroll = false;

function onStrumCreation(e) {
	var off = previousScroll != downscroll ? 10 : 0;
	...
	if (checkFileExists('songs/' + SONG.meta.name + '/noteskin.json')) {
		...
		if (skinName != null && checkFileExists('images/game/notes/' + skinName + '.json')) {
			...
			globalStrumOffsets[e.strumID + (e.player * 4)].statics.y = (skinData2.offsets.statics[1] ?? 0) + off;
	...
	for (si=> strumLine in strumLines.members) {
		for (i=> strumLine in strumLine.members) {
			if (strumLine.animation.name == 'static') {
				strumLine.x = defaultStrumPoses[i + (4 * si)].x + globalStrumOffsets[i + (4 * si)].statics.x;
		...
		strumLines.members[si].onHit.add((e) -> {
			...
			if (e.noteType != null || checkSongSkinData(si) != null) {
				...
				if (checkFileExists('images/game/notes/' + daType + '.json')) {
					...
					strumLines.members[si].members[e.direction].x = defaultStrumPoses[e.direction + (additiveNum * si)].x;
					...
					strumLines.members[si].members[e.direction].x += skinData.offsets.confirm[0] ?? 0;
					strumLines.members[si].members[e.direction].y += di(skinData.offsets.confirm[1] ?? 0) + (previousScroll != downscroll ? 10 : 0);
				}
				// e.cancelStrumGlow();
			} else {
